...
if ( location.protocol.substr(0,4)  === 'file' ||
	...
	$.ajax({
		...
		$( document ).on( "mobileinit", function() {
			...
			var message = $( '<div>' , {
				'class': "osa-content osa-fullwidth ui-bar-f",
...
$( document ).on( "pageinit", function() {
	...
	$( ".osa-version" ).html( html );
});
*/

$( document ).on( "pageinit", ".osa-demos", function() {
	var page = $( this );

	// global navmenu panel
	$( ".osa-navmenu-link" ).on( "click", function() {
		page.find( ".osa-navmenu-panel" ).panel( "open" );
	});

	// global search
	$( this ).find( ".osa-search ul.osa-list" ).listview({
		globalNav: "docs",
		inset: true,
		theme: "d",
		dividerTheme: "d",
	...
	$( this ).find( ".osa-header .osa-search-link" ).on( "click", function() {
		$( this ).parent( ".osa-header" ).toggleClass( "osa-search-toggle" );
		
		var type = $( this ).parent( ".osa-header" ).hasClass( "osa-search-toggle" ) ? "searchshow" : "searchhide";
		
		$( this ).parent( ".osa-header" ).find( ".osa-search" ).trigger( type );
	});
	
	$( this ).find( ".osa-header .osa-search" )
	...
	$( this ).on( "pagehide", function() {
		$( this ).find( ".osa-search .ui-input-clear" ).trigger( "click" );
	});

	$( this ).find( ".osa-content ul.osa-list " ).listview({
		globalNav: "docs",
		inset: true,
		theme: "d",
		dividerTheme: "d",
	...
	$( this ).find( ".osa-search-results-list li, .osa-search li" ).each(function() {
		var text = $( this ).attr( "data-filtertext" );
		$( this ).find( "a" ).append( "<span class='osa-search-results-keywords ui-li-desc'><span class='osa-keyword-hash'>//</span> " + text + "</span>" );
	});
});

$( document ).on( "pageshow",  ".osa-demos", function() {
	$( this ).find( ".osa-search input" ).attr( "autocomplete", "off" ).attr( "autocorrect", "off" );
});

$( document ).on( "pageshow", ".osa-demos-search-results", function() {
	var search = $.mobile.path.parseUrl( window.location.href ).search.split( "=" )[1], self = this;
	setTimeout(function() {
		e = $.Event( "keyup" );
		e.which = 65;
		$( self ).find( ".osa-content .osa-search-results-wrap input" ).val( search ).trigger(e).trigger( "change" );
	}, 0 );
});
/*
	...
	function innerHighlight( node, pat ) {
		...
		if ( node.nodeType == 3 ) {
			...
			if ( pos >= 0 ) {
				...
				spannode.className = "osa-search-results-highlight";
...
jQuery.fn.removeHighlight = function() {
	return this.find( "span.osa-search-results-highlight" ).each(function() {
...
$( document ).on( "mobileinit", function() {
	...
	$.widget( "mobile.listview", $.mobile.listview, {
		options: {
			...
			shadow: true,
			inset: false,
			initSelector: ":osaData(role='listview')",
			arrowKeyNav: false,
			enterToNav: false,
			highlight: false,
			submitTo: false
...
$( document ).on( "pageshow", ".osa-demos:not(.ui-page-header-fixed)", function() {
	var page = $( this ), 
		panelInnerHeight = page.find( ".osa-nav-panel.ui-panel-position-left" ).outerHeight() + 30,
